...
(function (PKT_EXT) {
    ...
    var experimentContainer = void 0,
        mainContainer = void 0,
        initalPosition = void 0,
        isActive = void 0,
        mouseInside = void 0,
    ...
    var closeSave = function closeSave(force) {
        if ($('.pkt_ext_component_experiment_wrapper:hover').length !== 0 && !force) return;

        stopCloseTimer();
        isActive = false;
        var main = $(experimentContainer);
    ...
    var getList = function getList(data) {
        log.debug('Get List', interfaceBuilt);
        ...
        if (!interfaceBuilt) {
            ...
            PKT_EXT.RECENT_SAVES.showPanel(data);
        }
    };

    var showArticleInfo = function showArticleInfo(data) {
        log.debug('Get Article Info', interfaceBuilt);

        if (!interfaceBuilt) {
            init(function () {
                return showArticleInfo(data);
            });
        } else {
            PKT_EXT.TWO_CLICK_SAVE.showPanel(data);
            PKT_EXT.TAGGING.setURL(data.item_preview.resolved_url);

            if (optionPremium && optionSuggestions) PKT_EXT.TAGGING.getSuggestions(data.item_preview.resolved_url);
        }
    };

    var articleSaved = function articleSaved(_ref) {
        var item_id = _ref.item_id,
            features = _ref.features,
            saveType = _ref.saveType;

        PKT_EXT.RECENT_SAVES.showPanel(features.saveList);
        // PKT_EXT.TWO_CLICK_SAVE.saveSuccess()
    ...
    var _sendAnalytics = function _sendAnalytics(type) {
        sendMessage({
            action: 'experiment',
            type: 'sendAnalytics',
            ctx: analytics[type]
    ...
    var init = function init(data) {
        if (!interfaceBuilt) {
            ...
            mainContainer = document.createElement('div');
            mainContainer.id = 'pkt_ext_master';
            mainContainer.className = 'pkt_ext_container';
            document.body.appendChild(mainContainer);

            experimentContainer = document.createElement('div');
            experimentContainer.id = 'pkt_experiment_save_plate';
            experimentContainer.className = 'pkt_ext_container_inactive pkt_ext_component_experiment';
            experimentContainer.innerHTML = PKT_EXT.TEMPLATES.experiments();

            mainContainer.appendChild(experimentContainer);

            // optionPremium           = (features && features.premstatus)
            // optionSuggestions       = (features && features.suggestions)

            PKT_EXT.TWO_CLICK_SAVE.init({
                containerId: 'pkt_ext_experiment_twoclick',
                onSaveCallback: function onSaveCallback() {
                    // experimentContainer.classList.add('pkt_ext_container_inactive')
                },
                onExitCallback: function onExitCallback() {
                    setTimeout(function () {
                        experimentContainer.classList.add('pkt_ext_container_inactive');
                    }, 500);
                },
                sendAnalytics: function sendAnalytics(type) {
                    _sendAnalytics(type);
                }
            });

            PKT_EXT.RECENT_SAVES.init({
                containerId: 'pkt_ext_experiment_recentsaves',
                sendAnalytics: function sendAnalytics(type) {
                    _sendAnalytics(type);
                }
            });
            ...
            PKT_EXT.TAGGING.init({
                ...
                closePanel: function closePanel() {
                    return experimentContainer.classList.add('pkt_ext_container_inactive');
                }
            });

            interfaceBuilt = true;
        } else if (!data.reusePanel) {
            $('#pkt_ext_master').remove();
            interfaceBuilt = false;
            return init();
        }

        setTimeout(function () {
            experimentContainer.classList.remove('pkt_ext_container_inactive');
        }, 1);
    };
    ...
    var messageReceived = function messageReceived(request) {

        switch (request.type) {
            case 'getArticleInfo':
                showArticleInfo(request.data);
                break;
    ...
    PKT_EXT.EXPERIMENT = { messageReceived: messageReceived, sendAnalytics: _sendAnalytics };
})(PKT_EXT || {});
...
(function (PKT_EXT) {
    ...
    var recentSaveData = {},
        recentSaveContainer = void 0,
        containerId = void 0,
        storedData = void 0,
        sendAnalytics = void 0;
    ...
    var checkOnboarding = function checkOnboarding(len) {
        var tmp = { show: len < 3 };
        if (len === 0) {
            tmp.zero = true;
        } else if (len === 1) {
    ...
    var initClickActions = function initClickActions() {
        $('.pkt_ext_recentsaves_cta').click(function (e) {
            e.preventDefault();
            sendAnalytics('viewList');

            var url = $(e.target).attr('href');
            window.open(url);
        });

        $('.pkt_ext_saveditem').click(function (e) {
            e.preventDefault();
            sendAnalytics('viewSavedItem');

            if ($(e.target).closest('li').hasClass('pkt_open_external_page')) {
                var url = $(e.target).closest('li').attr('rec_url');
                window.open(url);
            } else {
                var _url = $(e.target).closest('li').attr('href');
                window.open(_url);
            }
        });
    };
    ...
    var showPanel = function showPanel(data) {
        if (data) storedData = data;else data = storedData;

        // There was a network error, display error messaging
        if (data.error) {
            recentSaveContainer.innerHTML = PKT_EXT.TEMPLATES.recentsaves(data);
            return;
        }

        var listArray = Object.keys(data.list).map(function (key) {
            var currentItem = data.list[key];
            currentItem.domain = domainForURL(currentItem.resolved_url);
            currentItem.has_image = parseInt(currentItem.has_image, 10);

            if (currentItem.has_image == 1) {
                if (currentItem.images instanceof Array) {
                    currentItem.has_image = 0;
                } else {
                    var image_key = Object.keys(currentItem.images)[0];
                    currentItem.image_src = getImageCacheUrl(currentItem.images[image_key].src, 'w225');
                }
            }

            currentItem.rec_url = currentItem.given_url || currentItem.resolved_url;
            currentItem.external_site = currentItem.is_article !== '1' && currentItem.has_video !== '2' && currentItem.has_image !== '2';
        ...
        recentSaveData.onboarding = checkOnboarding(listArray.length);
        recentSaveData.list = listArray;
        recentSaveContainer.innerHTML = PKT_EXT.TEMPLATES.recentsaves(recentSaveData);
    ...
    var init = function init(options) {
        recentSaveContainer = document.getElementById(options.containerId);
        recentSaveContainer.innerHTML = PKT_EXT.TEMPLATES.recentsaves({ loading: true });

        if (options.storedList) {
            storedData = options.storedList;
        }

        sendAnalytics = options.sendAnalytics;
    };

    PKT_EXT.RECENT_SAVES = { init: init, showPanel: showPanel };
})(PKT_EXT || {});
...
(function (PKT_EXT) {
    ...
    var initClickActions = function initClickActions() {
        $('.pkt_ext_save_button').click(function (e) {
            e.preventDefault();

            $('#pkt_ext_experiment_twoclicksave').addClass('postsave');
            PKT_EXT.TOOLBAR.setMessage('Page saved!');

            sendMessage({
                action: 'experiment',
                type: 'saveLinkToPocket',
                actionInfo: {
                    cxt_view: 'extension',
                    cxt_ui: 'two_click'
                }
            ...
            // sendAnalytics('saveArticle')
        });

        $(document).click(function (e) {
            var container = $('.pkt_ext_component_experiment_wrapper');

            // if the target of the click isn't the container nor a descendant of the container or a taggle element
            if (!container.is(e.target) && container.has(e.target).length === 0 && $(e.target).closest('li.taggle.taggle_hot').length !== 1) {
                e.preventDefault();
                onExitCallback();
                sendAnalytics('dismissOverlay');
    ...
    var showPanel = function showPanel(data) {
        PKT_EXT.TOOLBAR.setItemId(data.item_preview.item_id);

        twoClickData = {
            domain: data.item_preview.resolved_domain,
            title: data.item_preview.title,
            resolvedUrl: data.item_preview.resolved_url,
            has_image: data.item_preview.top_image_url ? 1 : 0
        };

        if (data.item_preview.top_image_url) {
            twoClickData.image_src = getImageCacheUrl(data.item_preview.top_image_url, 'w225');
        }

        initClickActions();

        document.getElementById('pkt_ext_articleinfo').innerHTML = PKT_EXT.TEMPLATES.articleinfo(twoClickData);
    };

    var init = function init(options) {
        twoClickContainer = document.getElementById(options.containerId);
        twoClickContainer.innerHTML = PKT_EXT.TEMPLATES.twoclicksave(PKT_EXT.TRANS);
        ...
        PKT_EXT.TOOLBAR.init({
            closePanel: onExitCallback
        });

        PKT_EXT.TOOLBAR.setMessage('Save Page to Pocket');
    };

    PKT_EXT.TWO_CLICK_SAVE = { init: init, showPanel: showPanel /*saveSuccess*/ };
})(PKT_EXT || {});
