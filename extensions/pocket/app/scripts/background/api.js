...
(function(PKT_EXT) {
    ...
    /**
     * Get article info from parser to display as formatted item
     */
    function getArticleInfo(options) {
        apiRequest({
            path: '/getItemPreview',
            data: {
                access_token: getSetting('oauth_token'),
    ...
    /**
     * Get three most recently saved articles for user
     */
    ...
    function sendExperimentAnalytics(ctx, callback) {
        var guid = getSetting('guid')
        if (typeof guid == 'undefined'){
            PKT_EXT.API.getGuid(function(data) {
                if (data.guid) {
                    setSetting('guid', data.guid)
                    sendExperimentAnalytics(ctx, callback)
                    log.info('Not Enough Data - Rechecking') // Should be split into opposing functions
                } else {
                    log.info('Not Enough Data - Terminating')
                }
    ...
    PKT_EXT.API = {
        ...
        sendExperimentAnalytics     : sendExperimentAnalytics
    }
}(PKT_EXT || {}))
