...
(function() {
    ...
    function updateUI() {
        ...
        sendMessage({action: "getSetting", key: "username"}, function(response) {
            ...
            sendMessage({action: "getSetting", key: "show_recs"}, function(response) {
                // DISABLING THIS FOR TWO-CLICK
                // if(response && response.value == 'true'){
                //     $('#ril-recommendations').show();
                // }

                sendMessage({action: "getSetting", key: "show_chrome_two_click"}, function(response2) {
                    if(response && response.value == 'true' && (response2.value === undefined || response2.value == 'false')){
                        $('#ril-recommendations').show();
                    }
                });
            });
            ...
            // $('#ril-twoclick').hide();
            // sendMessage({action: "getSetting", key: "show_chrome_two_click"}, function(response) {
            //     // showing automatically for now
            //     // $('#ril-twoclick').show();
            //     $('#twoclick-checkbox').prop('checked', boolFromString(response.value));
            // });
            ///////////////////////
    ...
    function initCheckboxes() {
        ...
        // TWO-CLICK EXPERIMENT
        // $('#twoclick-checkbox').on('click', function() {
        //     let isChecked = stringFromBool($(this).is(':checked'));
        //     sendMessage({action:"setSetting", key: "show_two_click", value:isChecked});
        //     sendMessage({action:"experiment", type: "sendAnalytics", ctx: { action:'pv_wt', view:'extension', section:'two_click', identifier:(isChecked) ? 'toggle_on' : 'toggle_on' }});
        // });
        ///////////////////////
